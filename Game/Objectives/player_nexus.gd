...
var text = "Guard the Friendly Nexus"
onready var root_node = get_node("/root/TestLevel")
onready var player = root_node.get_node("Human Player")
...
func _ready():
	randomize()
	guard_time= randi()%100 + 40
	var timeout = guard_time* 2
...
func _process(delta):
	...
	dist = get_pos().distance_to(player.get_pos())
	if dist < guard_dist:
		currentTime+=delta
	else:
		pass

	if currentTime >= guard_time:
...
func _draw():
	...
	draw_rect(Rect2(50,50,160.0 *currentTime/guard_time, 40),  Color(0.18, 0.475, 0.729))
	...
	if dist < guard_dist:
		draw_circle_arc( center, guard_dist, angle_from, angle_to, color )
	else:
		draw_circle_arc( center, guard_dist, angle_from, angle_to, Color(0.871, 0.808, 0.612) )
